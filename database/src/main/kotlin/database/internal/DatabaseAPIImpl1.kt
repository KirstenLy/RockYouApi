...
internal class DatabaseAPIImpl1(
    db: DBTest,
    ...
    private val testEx = TestReportRequestExecutor(db)
    ...
    override suspend fun getPictureByID(filter: PictureByIDFilter): SimpleOptionalDataResult<Picture> =
        getPictureByIDRequestExecutor.execute(filter)
    ...
    override suspend fun report(contentID: Int, reason: String, userID: Int?): SimpleUnitResult {
        testEx.execute(Random.nextInt(), contentID, reason, userID)
